...
public class Copilot implements PlayableHandsFinder{
	...
	PokerHandProbabilityTable[] discardTables = new PokerHandProbabilityTable[5];
	BitSet updatedTables = new BitSet(5);
	 ...
	 */
	public List<PokerHandProbabilityTable> analyzeGameState(GameState gameState, int numSelectedCards) {
		...
		List<PokerHandProbabilityTable> discardTables = scanDiscards(gameState, numSelectedCards);
		
		// If there are no selected cards, 5 tables have been generated, and it should loop through them
		...
		// If there were selected cards, it only generated one table
		else{
			analysis = CopilotMessage.printProbabilityTableFromDiscard(gameState.getCurrDeck().drawnCards, numSelectedCards, discardTables.get(0).printTable());
			s.append(analysis).append("\n");
			System.out.println(analysis);
		}
	...
	public List<PokerHandProbabilityTable> scanDiscards(GameState game, int numSelectedCards){
		updatedTables.clear();
		...
		if(numSelectedCards > 0) {
			...
			System.out.println("Num Discarded - " + numSelectedCards);
			p = new PotentialHandsFinder(game);
			
			PokerHandProbabilityTable table = p.generateProbabilityTableOfPotentialHands(game, numSelectedCards);
			discardTables.add(table);
			table.name = String.format("%d discarded", numSelectedCards);
			
			if(updateCopilotPanel) copilotPanel.addTab(table, table.name);
		}else {
			for(int i = 1; i <= 5; i++) {
				if (Thread.currentThread().isInterrupted()) {
		            // Clean up and exit early
